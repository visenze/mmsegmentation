...
"""
This profiler tool could run an onnx model and print out the kernel time spent on each Node of the model.
Example of profiling of an onnx model:
    python profiler.py --model model.onnx --batch_size 1 --samples 1000 --thread_num 8 --input_shape longformer --use_gpu
"""
...
def parse_arguments(argv=None):
    ...
    parser.add_argument(
        "-m", "--model", required=True, type=str, help="onnx model path"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="comma-separated shape of the input. E.g.: 1,3,1024,1024 ",
    )
...
def group_profile_results(sess_time, kernel_time_only, use_gpu):
    ...
    for item in sess_time:
        if (
            ...
            is_cpu = (
                "provider" in item["args"]
                and item["args"]["provider"] == "CPUExecutionProvider"
            )
            if is_cpu:
    ...
    if use_gpu:
        lines = [
            "Average(μs)\tTotal(μs)\tTotal_Percentage\tCalls\tCpu_Duration\tCpu_Calls\tName"
        ]
    else:
    ...
    for op_name, duration in sorted(op_time.items(), key=lambda x: x[1], reverse=True):
        ...
        if use_gpu:
            ...
            lines.append(
                f"{avg_time:.1f}\t{duration}\t{ratio * 100.0:5.2f}\t{calls}\t{op_name}"
            )
...
def create_dummy_inputs(shape, data_type, samples=1000):
    ...
    all_inputs = [data for _ in range(samples)]
    return all_inputs
...
def run(args):
    ...
    all_inputs = create_dummy_inputs(shape, np.float32)

    profile_file = run_profile(
        args.model, args.use_gpu, args.basic_optimization, args.thread_num, all_inputs
    )

    profile_records = load_profile_json(profile_file)

    lines = parse_profile_results(
        profile_records, args.kernel_time_only, args.threshold
    )
